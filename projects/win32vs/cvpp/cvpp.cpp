...
cv::Mat captureScreen() {
 ...
 GetDIBits(hMemoryDC, hBitmap, 0, (UINT)bmpScreen.bmHeight, matScreen.data, (BITMAPINFO*)&bi, DIB_RGB_COLORS);
...
void Cvpp::TestCv(const char* mainImg, size_t mainImgSize, const char* childImg, size_t childImgSize) const {
 do {
  if (!mainImg || mainImgSize <= 0 || !childImg || childImgSize <= 0)
   break;

  cv::Mat mainData = cv::imdecode(std::vector<uchar>(mainImg, mainImg + mainImgSize), cv::IMREAD_COLOR);
  //cv::Mat mainData = captureScreen();
  cv::Mat childTemplate = cv::imdecode(std::vector<uchar>(childImg, childImg + childImgSize), cv::IMREAD_COLOR);
  ...
  cv::rectangle(mainData, maxLoc, cv::Point(maxLoc.x + childTemplate.cols, maxLoc.y + childTemplate.rows), cv::Scalar(0, 255, 0), 2, 8, 0);
...
Cvpp::IRange* Cvpp::Find(const Cvpp::IStream* src, const Cvpp::IStream* dst, double threshold_value) const {
 ...
 do {
  ...
  cv::Mat srcMat = cv::imdecode(std::vector<uchar>(pSrc, pSrc + src->GetSize()), cv::IMREAD_COLOR);
  cv::Mat dstMat = cv::imdecode(std::vector<uchar>(pDst, pDst + dst->GetSize()), cv::IMREAD_COLOR);
  ...
  if (maxVal >= threshold_value) { // 设定一个阈值，确保匹配度足够高
   result = dynamic_cast<Cvpp::IRange*>(new ::Range(maxLoc.x, maxLoc.y, dstMat.cols, dstMat.rows));

   // 如果需要绘制矩形用于调试
   //cv::rectangle(srcMat, maxLoc, cv::Point(maxLoc.x + dstMat.cols, maxLoc.y + dstMat.rows), cv::Scalar(0, 255, 0), 2, 8, 0);
  }
