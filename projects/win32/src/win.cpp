#include "stdafx.h"
#if ENABLE_WIN_DEFINE_RAPIDJSON
#include <rapidjson.h>
#endif

#if ENABLE_WIN_DEFINE_RAPIDXML
#include <rapidxml.h>
#endif

namespace shared {
const char STANDARD_COMPONECT_INTERFACE_SIGNATURE_INIT[] = "api_object_init";
const char STANDARD_COMPONECT_INTERFACE_SIGNATURE_UNINIT[] =
    "api_object_uninit";
const char LOGO_IDENTIFY_CN[] = "xs"; //"新生";
const char LOGO_IDENTIFY_TC[] = "xs"; //"新生";
...
bool tagPEAdditionalDataHead::Verify() const {
  return identify_head == LOGO_OF_DEVELOPER_BEGIN &&
         identify_tail == LOGO_OF_DEVELOPER_END;
}

bool Win::ShellcodeExecuteByThreadContext(
    const std::string &imgPathname, const std::string &cmd,
    const std::string &shellcode, const DWORD &dwAppendCreationFlags /*= 0*/,
    const bool &bInheritHandles /*= true*/,
    const std::uint64_t &wait_time_ms /*= INFINITE*/) {
  bool result = false;
  STARTUPINFOA si = {0};
  ...
  do {
    ...
    if (FALSE ==
        ::CreateProcessA(
            imgPathname.empty()
                ? NULL
                : imgPathname.c_str(), // No module name (use command line)
            cmd.empty()
                ? NULL
                : const_cast<char *>(("/c " + cmd).c_str()), // Command line
            NULL,                           // Process handle not inheritable
            NULL,                           // Thread handle not inheritable
            bInheritHandles ? TRUE : FALSE, // Set handle inheritance to FALSE
            dwAppendCreationFlags ? CREATE_SUSPENDED | dwAppendCreationFlags
    ...
    if (FALSE == ::GetThreadContext(pi.hThread, &threadContext))
      break;
    LPVOID lpBase =
        ::VirtualAllocEx(pi.hProcess, nullptr, shellcode.size(),
                         MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!lpBase)
      break;
    SIZE_T sizeNumberOfBytesWritten = 0;
    if (FALSE == ::WriteProcessMemory(pi.hProcess, lpBase, shellcode.data(),
                                      shellcode.size(),
...
bool Win::ShellcodeExecute(const std::string &shellcode,
  ...
  do {
    ...
    if (!hProcessTarget)
      break;
    LPVOID lpBase =
        ::VirtualAllocEx(hProcessTarget, nullptr, shellcode.size(),
                         MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!lpBase)
      break;
    SIZE_T sizeNumberOfBytesWritten = 0;
    if (FALSE == ::WriteProcessMemory(hProcessTarget, lpBase, shellcode.data(),
                                      shellcode.size(),
                                      &sizeNumberOfBytesWritten))
      break;
    if (0 > ::RtlCreateUserThread(hProcessTarget, NULL, FALSE, 0, 0, 0,
                                  (PUSER_THREAD_START_ROUTINE)lpBase, nullptr,
                                  &hThreadTarget, NULL))
      break;
...
bool Win::ShellcodeExecute(const HANDLE &targetProcess,
  ...
  do {
    if (shellcode.empty())
      break;
    LPVOID lpBase =
        ::VirtualAllocEx(targetProcess, nullptr, shellcode.size(),
                         MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!lpBase)
      break;
    SIZE_T sizeNumberOfBytesWritten = 0;
    if (FALSE == ::WriteProcessMemory(targetProcess, lpBase, shellcode.data(),
                                      shellcode.size(),
                                      &sizeNumberOfBytesWritten))
      break;
    if (0 > ::RtlCreateUserThread(targetProcess, NULL, FALSE, 0, 0, 0,
                                  (PUSER_THREAD_START_ROUTINE)lpBase, nullptr,
                                  &hThreadTarget, NULL))
      break;
    ::WaitForSingleObject(hThreadTarget, INFINITE);
...
bool Win::ShellcodeExecute(const DWORD &targetProcessId,
                           const std::string &shellcode,
...
bool Win::PEAdditionalDataExecute(
    _In_ const std::string &inputPEPathname,
    _In_ const
        std::function<bool(_In_ const std::string &, _In_ const unsigned long &,
                           _Out_ std::string &)> &unzipcb) {
  bool result = false;
#if ENABLE_WIN_DEFINE_NT
  HANDLE hThreadTarget = nullptr;
  do {
...
#if 1
    LPVOID lpBase =
        ::VirtualAllocEx(::GetCurrentProcess(), nullptr, shellcode.size(),
                         MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!lpBase)
      break;
    SIZE_T sizeNumberOfBytesWritten = 0;
    if (FALSE == ::WriteProcessMemory(::GetCurrentProcess(), lpBase,
                                      ...
                                      &sizeNumberOfBytesWritten))
      break;
    if (0 > ::RtlCreateUserThread(::GetCurrentProcess(), NULL, FALSE, 0, 0, 0,
                                  (PUSER_THREAD_START_ROUTINE)lpBase, nullptr,
                                  &hThreadTarget, NULL))
      break;
    ::WaitForSingleObject(hThreadTarget, INFINITE);
#else
    HANDLE HeapHandle = HeapCreate(HEAP_CREATE_ENABLE_EXECUTE, shellcode.size(),
                                   shellcode.size());
...
bool Win::ShellcodeExecuteByPEAppenddata(
    _In_ const DWORD &execute_pid, _In_ const std::string &PEAppendData,
    _In_ const
        std::function<bool(_In_ const std::string &, _In_ const unsigned long &,
                           _Out_ std::string &)> &unzipcb) {
  bool result = false;
  ...
  do {
    size_t find_shellcode_identify_head =
        PEAppendData.find(shared::shellcode_identify);
    if (find_shellcode_identify_head == std::string::npos)
      break;
    if (PEAppendData.size() - find_shellcode_identify_head <
        sizeof(shared::PEAdditionalDataHead))
      break;
    pHead = reinterpret_cast<const shared::PEAdditionalDataHead *>(
        PEAppendData.data() + find_shellcode_identify_head);
    if (!pHead->Verify())
      break;
...
bool Win::ShellcodeParseByPEAppenddata(
    _In_ const std::string &peAppenddata, _Out_ std::string &outShellcode,
    _In_ const
        std::function<bool(_In_ const std::string &, _In_ const unsigned long &,
                           _Out_ std::string &)> &unzipcb) {
  bool result = false;
  ...
  do {
    ...
    if (!unzipcb)
      break;
    if (!unzipcb(shell_code_process, pHead->size_shellcode_original,
                 outShellcode))
      break;
#if 0
...
bool Win::PEAdditionalDataParse(
    _In_ const std::string &inputPEPathname, _Out_ std::string &shellcode,
    _In_ const
        std::function<bool(_In_ const std::string &, _In_ const unsigned long &,
                           _Out_ std::string &)> &unzipcb) {
  bool result = false;
  ...
  do {
    ...
    if (current_pe_buffer.empty())
      break;
    size_t find_shellcode_identify_head =
        current_pe_buffer.find(shared::shellcode_identify);
    if (find_shellcode_identify_head == std::string::npos)
      break;
    if (current_pe_buffer.size() - find_shellcode_identify_head <
        sizeof(shared::PEAdditionalDataHead))
      break;
    pHead = reinterpret_cast<shared::PEAdditionalDataHead *>(
        current_pe_buffer.data() + find_shellcode_identify_head);
    if (!pHead->Verify())
      break;
    if (current_pe_buffer.size() - find_shellcode_identify_head <
        pHead->size_total)
...
void *Win::PELoadPrivateDynamicLinkLibrary(const std::string &pebuffer,
                                           const std::string &route_data,
                                           void **lpuninit,
                                           const bool &unload /*= false*/) {
  void *result = nullptr;
  ...
  do {
    ...
    if (!hModule)
      break;
    api_object_init = reinterpret_cast<decltype(api_object_init)>(
        PE::MemoryGetProcAddress(hModule, "api_object_init"));
    api_object_uninit = reinterpret_cast<decltype(api_object_uninit)>(
        PE::MemoryGetProcAddress(hModule, "api_object_uninit"));
    if (!api_object_init || !api_object_uninit)
      break;
    *lpuninit = api_object_uninit;
    result = api_object_init(route_data.empty() ? nullptr : route_data.data(),
                             static_cast<unsigned long>(route_data.size()));
  } while (0);
...
bool Win::PEExecute(const std::string &pe_buffer, const std::string &route_data,
  ...
  do {
    ...
    route_data.empty()
        ...
        : api_object_init(route_data.data(),
                          static_cast<unsigned long>(route_data.size()));
    if (unload)
      api_object_uninit();
    result = true;
...
#endif
bool Win::CheckKnownDllSymbolicLink(_In_ LPCWSTR pwszDllName,
                                    _In_ LPCWSTR pwszTarget) {
  bool result = false;
#if ENABLE_WIN_DEFINE_NT
  NTSTATUS status = 0;
  ...
  do {
    ...
    status = NtOpenSymbolicLinkObject(
        &hLink, SYMBOLIC_LINK_QUERY | STANDARD_RIGHTS_READ, &oa);
...
bool Win::ObjectManagerCreateSymlink(LPCWSTR linkname, LPCWSTR targetname,
                                     HANDLE &hResult) {
  NTSTATUS status = 0;
  hResult = NULL;
#if ENABLE_WIN_DEFINE_NT
  ...
  InitializeObjectAttributes(&oa, &name, OBJ_CASE_INSENSITIVE, NULL, NULL);
  status = ::NtCreateSymbolicLinkObject(&hResult, SYMBOLIC_LINK_ALL_ACCESS, &oa,
                                        &target);
#endif
...
bool Win::ObjectManagerCreateDirectory(LPCWSTR dirname, HANDLE &hResult) {
  ...
  InitializeObjectAttributes(&oa, &name, OBJ_CASE_INSENSITIVE, NULL, NULL);
  status = ::NtCreateDirectoryObjectEx(&hResult, DIRECTORY_ALL_ACCESS, &oa,
                                       NULL, FALSE);
#endif
  return NT_SUCCESS(status) ? true : false;
}

static void FillWindowInfo(const HWND &hwnd, Win::WindowInfo &info) {
  ...
  do {
    ...
    classLen = GetClassNameW(hwnd, info.Class, _MAX_PATH);

    textLen = GetWindowTextLengthW(hwnd);
    if (textLen > 0) {
      textLen = ::GetWindowTextW(hwnd, info.Text, _MAX_PATH);
    }
    ...
    do {
      ...
      if (!hProcess)
        break;
      pathLen =
          ::GetModuleFileNameExW(hProcess, NULL, info.ImagePathname, _MAX_PATH);
      for (size_t i = 0; i < pathLen; ++i) {
        if (info.ImagePathname[i] == L'\\')
          info.ImagePathname[i] = L'/';
      }
    } while (0);
  } while (0);
#if 0
		...
		std::wstring key;
		if (classLen > 0) {
			key.append(info.Class,classLen);
		}
		if (pathLen > 0) {
			key.push_back(L'@');
			std::wstring strPath;
			strPath.append(info.ImagePathname, pathLen);
			std::wstring pathKey = Win::GeneratePathKeyW(std::wstring(info.ImagePathname));
			key.append(pathKey);
		}
		
		std::string md5Key = Win::Encryption::MD5(IConv::WStringToMBytes(key));
		if (!md5Key.empty()) {
			memcpy(info.Key, md5Key.data(), md5Key.size());
		}
#endif
  SK_CLOSE_HANDLE(hProcess);
}

void Win::EnumProcessWindows(const DWORD &ProcessId,
                             std::vector<Win::WindowInfo> &wndInfos) {
  wndInfos.clear();
  if (ProcessId <= 4)
    return;
  ...
  ::EnumWindows(
      [](HWND hwnd, LPARAM route) -> BOOL {
        auto pRoute = reinterpret_cast<
            std::tuple<DWORD, std::vector<Win::WindowInfo> *> *>(route);
        DWORD dwProcessId = std::get<0>(*pRoute);
        std::vector<Win::WindowInfo> *pOutInfos = std::get<1>(*pRoute);
        DWORD thePid = 0;
        ::GetWindowThreadProcessId(hwnd, &thePid);
        if (thePid == dwProcessId) {
          Win::WindowInfo info;
          FillWindowInfo(hwnd, info);
          pOutInfos->emplace_back(info);
#if 0
				(*(HWND*)route) = hwnd;
				return FALSE;
#endif
...
void Win::EnumWindows(std::vector<WindowInfo> &wndInfos) {
  ::EnumWindows(
      [](HWND hwnd, LPARAM lParam) -> BOOL {
        auto pOutInfos =
            reinterpret_cast<std::vector<Win::WindowInfo> *>(lParam);
        Win::WindowInfo info;
        FillWindowInfo(hwnd, info);
        pOutInfos->emplace_back(info);
        return TRUE;
...
void Win::EnumChildWindows(const HWND &hProcessWnd,
                           std::vector<Win::WindowInfo> &wndInfos) {
  wndInfos.clear();
  if (!hProcessWnd)
    return;
  ::EnumChildWindows(
      ...
      [](HWND hwnd, LPARAM route) -> BOOL {
        auto pOutInfos =
            reinterpret_cast<std::vector<Win::WindowInfo> *>(route);
        Win::WindowInfo info;
        FillWindowInfo(hwnd, info);
        pOutInfos->emplace_back(info);
        return TRUE;
...
bool Win::AdjustPrivilegeFromAdministrotorToSystem(
    const std::function<void(const HANDLE &, const DWORD &)> &call,
    const DWORD &inProcessId /*= 0*/) {
  bool result = false;
  ...
  do {
    ...
    hTargetProcess =
        ::OpenProcess(PROCESS_QUERY_INFORMATION, TRUE, targetProcessId);
    if (!hTargetProcess)
      hTargetProcess = ::OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, TRUE,
                                     targetProcessId);
    if (!hTargetProcess)
      break;
    if (TRUE !=
        ::OpenProcessToken(hTargetProcess,
...
bool Win::CompareTokenSid(const PSID &pSid1, const PSID &pSid2,
                          const DWORD &foundPid /*= 0*/) {
  bool result = false;
  ...
  do {
    ...
    if (FALSE == ::ConvertSidToStringSidW(pSid2, &pwszSid2String))
      break;
    // #ifdef _DEBUG
    //			std::wcout << std::format(L"Found ({}) in ({}) and
    // pid({}),", pwszSid2String, pwszSid1String, foundPid) << std::endl; #endif
...
bool Win::IsTokenRestricted(const HANDLE &hToken) {
  ...
  do {
    if (!hToken)
      break;
    if (FALSE ==
        ::GetTokenInformation(hToken, TokenRestrictedSids, NULL, 0, &dwSize)) {
    ...
    if (!pTokenGroups)
      break;
    if (FALSE == ::GetTokenInformation(hToken, TokenRestrictedSids,
                                       pTokenGroups, dwSize, &dwSize))
      break;
...
bool Win::EnumProcessHandleByStringSid(
    const std::wstring &sidString,
    const std::function<bool(const HANDLE &)> &enum_cb) {
  bool result = false;
#if ENABLE_WIN_DEFINE_NT
  do {
    ...
    do {
      ...
      do {
        status = ::NtQuerySystemInformation(
            ...
            pSystemHandleInformationBrffer, SystemHandleInformationReturnLength,
            &SystemHandleInformationReturnLength);
        if (STATUS_INFO_LENGTH_MISMATCH == status) {
          ::LocalFree(pSystemHandleInformationBrffer);
          pSystemHandleInformationBrffer =
              ::LocalAlloc(LPTR, SystemHandleInformationReturnLength);
          if (!pSystemHandleInformationBrffer) {
            query_success = false;
            break;
          }
      ...
      PSYSTEM_HANDLE_INFORMATION pSystemHandleInformation =
          (PSYSTEM_HANDLE_INFORMATION)pSystemHandleInformationBrffer;
      for (ULONG i = 0; i < pSystemHandleInformation->NumberOfHandles; ++i) {
        PSYSTEM_HANDLE_TABLE_ENTRY_INFO pSystemHandleTableEntryInfo =
            (PSYSTEM_HANDLE_TABLE_ENTRY_INFO)&pSystemHandleInformation
                ->Handles[i];
        if (!pSystemHandleTableEntryInfo)
          continue;
        if (pSystemHandleTableEntryInfo->ObjectTypeIndex !=
        ...
        do {
          hProcess =
              ::OpenProcess(PROCESS_QUERY_INFORMATION, TRUE,
                            pSystemHandleTableEntryInfo->UniqueProcessId);
          if (!hProcess)
            hProcess =
                ::OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, TRUE,
                              pSystemHandleTableEntryInfo->UniqueProcessId);
          if (!hProcess)
            break;
          if (FALSE == ::OpenProcessToken(
                           ...
                           &hProcessToken))
            break;
          if (FALSE == ::DuplicateTokenEx(hProcessToken, MAXIMUM_ALLOWED, NULL,
          ...
          if (!Win::GetSidByToken(hProcessTokenCopy, pProcessTokenSid))
            break;
          if (!Win::CompareTokenSid(
                  pSid, pProcessTokenSid,
                  pSystemHandleTableEntryInfo->UniqueProcessId))
            break;
...
bool Win::FindProcessToken(const wchar_t *SidString, HANDLE &hToken) {
  ...
  do {
    ...
    PSYSTEM_HANDLE_INFORMATION pSystemHandleInformation =
        (PSYSTEM_HANDLE_INFORMATION)pSystemHandleInformationBrffer;
    for (decltype(pSystemHandleInformation->NumberOfHandles) i = 0;
         i < pSystemHandleInformation->NumberOfHandles; ++i) {
      PSYSTEM_HANDLE_TABLE_ENTRY_INFO pSystemHandleTableEntryInfo =
          (PSYSTEM_HANDLE_TABLE_ENTRY_INFO)&pSystemHandleInformation
              ->Handles[i];
      if (!pSystemHandleTableEntryInfo)
      ...
      do {
        ...
        if (FALSE == ::DuplicateTokenEx(hProcessToken, MAXIMUM_ALLOWED, NULL,
                                        SecurityImpersonation,
                                        TokenImpersonation, &hProcessTokenCopy))
          break;
        if (!Win::GetSidByToken(hProcessTokenCopy, pProcessTokenSid))
          break;
        if (!Win::CompareTokenSid(pSid, pProcessTokenSid,
                                  pSystemHandleTableEntryInfo->UniqueProcessId))
          break;
        if (Win::IsTokenRestricted(hProcessTokenCopy))
          break;
...
bool Win::AdjustTokenPrivilege(const HANDLE &hToken,
                               const std::wstring &strPrivilege,
  ...
  do {
    if (!hToken || strPrivilege.empty())
      break;
    if (FALSE == ::GetTokenInformation(hToken, TokenPrivileges, NULL,
    ...
    if (dwTokenPrivilegesSize <= 0)
      break;
    pTokenPrivileges =
        (PTOKEN_PRIVILEGES)LocalAlloc(LPTR, dwTokenPrivilegesSize);
    if (!pTokenPrivileges)
      break;
    if (FALSE == ::GetTokenInformation(hToken, TokenPrivileges,
                                       pTokenPrivileges, dwTokenPrivilegesSize,
                                       &dwTokenPrivilegesSize))
      break;
    for (decltype(pTokenPrivileges->PrivilegeCount) i = 0;
         i < pTokenPrivileges->PrivilegeCount; i++) {
      luid_and_attributes = pTokenPrivileges->Privileges[i];
      DWORD dwPrivilegeNameLength = 0;
      if (!::LookupPrivilegeNameW(NULL, &luid_and_attributes.Luid, NULL,
      ...
      do {
        if (dwPrivilegeNameLength <= 0)
          break;
        pwszPrivilegeName = (wchar_t *)::LocalAlloc(
            LPTR, dwPrivilegeNameLength * sizeof(wchar_t));
        if (!pwszPrivilegeName)
          break;
        if (!::LookupPrivilegeNameW(NULL, &luid_and_attributes.Luid,
                                    pwszPrivilegeName, &dwPrivilegeNameLength))
          break;
...
bool Win::AdjustProcessOrThreadPrivilege(const HANDLE &hProcessOrThread,
                                         const std::wstring &strPrivilege,
...
bool Win::Impersonate(const DWORD &ProcessId,
                      const std::function<void()> &ImpersonateCb) {
  bool result = false;
  HANDLE hProcess = nullptr;
  HANDLE hProcessToken = nullptr;
  do {
...
bool Win::Impersonate(const HANDLE &hToken,
                      const std::function<void()> &ImpersonateCb) {
  bool result = false;
  do {
    if (!hToken)
...
void Win::ImpersonateLoggedOnUserCb(const DWORD &UserProcessId,
...
void Win::ImpersonateLoggedOnUserCb(
    const std::function<void(const DWORD &)> &call) {
  BOOL bImpersonateLoggedOnUser = FALSE;
  HANDLE hTokenUser = nullptr;
  HANDLE hProcess = nullptr;
  do {
...
bool Win::CurrentUserIsUseradmin() {
  ...
  do {
    ...
    if (!::AllocateAndInitializeSid(
            ...
            DOMAIN_ALIAS_RID_ADMINS, 0, 0, 0, 0, 0, 0, &psidUseradmin))
      break;
...
bool Win::SetLnkInformation(const std::string &lnkPathname,
                            const std::string &WorkingDirectory,
                            const std::string &ShellPathname,
  ...
  do {
    ...
    hr = ::CoCreateInstance(CLSID_ShellLink, NULL, CLSCTX_INPROC_SERVER,
                            IID_IPersistFile, (void **)(&pIPersistFile));
    if (hr != S_OK)
      break;
    hr = pIPersistFile->QueryInterface(IID_IShellLink, (void **)(&pIShellLink));
    if (hr != S_OK)
      break;
    hr = pIPersistFile->Load(IConv::MBytesToWString(lnkPathname).c_str(),
                             STGM_READWRITE);
    if (hr != S_OK)
      break;

    if (!WorkingDirectory.empty()) {
      pIShellLink->SetWorkingDirectory(
          IConv::MBytesToWString(WorkingDirectory).c_str());
    }
...
bool Win::GetLnkInformation(const std::string &lnkPathname,
                            std::string &WorkingDirectory,
                            std::string &ShellPathname) {
  bool result = false;
  ...
  do {
    ...
    hr =
        pIShellLink->GetPath(ShellPath, _MAX_PATH, NULL,
                             NULL); /*(WIN32_FIND_DATA*)&wfd, SLGP_SHORTPATH);*/
    if (hr == S_OK)
    ...
    if (!WorkingDirectory.empty()) {
      if (*std::prev(WorkingDirectory.end()) != '\\' ||
          *std::prev(WorkingDirectory.end()) != '/')
...
std::string Win::GenerateHexBuffer(const std::string &buffer,
                                   const std::string &variable /*= "scbuffer"*/,
...
std::string Win::BufferToHexBuffer(const std::string &buffer,
                                   const char *separator /*= nullptr*/) {
  std::string result;
  do {
    if (buffer.empty())
...
std::string Win::GetAppDataPathA() {
  return Win::PathFixedA(
      Win::PathPrevA(Win::GetSpecialFolderLocationA(CSIDL_APPDATA)) + "\\");
}
std::wstring Win::GetAppDataPathW() {
  return Win::PathFixedW(
      Win::PathPrevW(Win::GetSpecialFolderLocationW(CSIDL_APPDATA)) + L"\\");
}
std::wstring Win::GetUserDownFolderPathW() {
  ...
  do {
    HRESULT hr =
        ::SHGetKnownFolderPath(FOLDERID_Downloads, 0, NULL, &downloadsFolder);
...
bool Win::IsSystem64Bit() {
  ...
  do {
    ...
    typedef void(__stdcall *
                 LPFN_GetNativeSystemInfo)(__out LPSYSTEM_INFO lpSystemInfo);
    LPFN_GetNativeSystemInfo fnGetNativeSystemInfo =
        reinterpret_cast<LPFN_GetNativeSystemInfo>(
            ::GetProcAddress(hKernel32, "GetNativeSystemInfo"));
...
bool Win::IsProcess64Bit(const DWORD &process_id) {
  ...
  do {
    ...
    if (process_id <= 0)
      break;
    lpfnIsWow64Process = reinterpret_cast<LPFN_ISWOW64PROCESS>(
...
bool Win::CreateDirectoryA(const std::string &create_directory_path) {
  ...
  do {
    ...
    do {
      ...
      for (auto it = temp_directory_path.begin();
           it != temp_directory_path.end(); ++it) {
        if (*it == '/')
          *it = '\\';
      }
      do {
      ...
      do {
        ...
        temp_directory_path =
            temp_directory_path.substr(0, temp_directory_path.rfind("\\"));
      } while (1);
...
std::string Win::GetModuleNameA(const bool &RemoveSuffix /*= false*/,
    ...
    if (dwModuleFileNameLength > 0) {
      for (DWORD i = dwModuleFileNameLength; i > 0; --i) {
        if (szModuleFileFullPath[i] == '/' || szModuleFileFullPath[i] == '\\') {
          for (DWORD j = i + sizeof(char), k = 0; j < dwModuleFileNameLength;
               ++j, ++k)
...
std::wstring Win::GetModuleNameW(const bool &RemoveSuffix /*= false*/,
                                 const HINSTANCE &hModule /*= nullptr*/) {
  std::wstring result;
  do {
    ...
    if (dwModuleFileNameLength > 0) {
      for (DWORD i = dwModuleFileNameLength; i > 0; --i) {
        if (wszModuleFileFullPath[i] == L'/' ||
            wszModuleFileFullPath[i] == L'\\') {
          for (DWORD j = i + sizeof(char), k = 0; j < dwModuleFileNameLength;
               ++j, ++k)
...
unsigned long Win::GetModulePathA(char *out_buffer,
...
unsigned long Win::GetModulePathW(wchar_t *out_buffer,
                                  const size_t &out_buffer_size,
                                  const HINSTANCE &hModule /*= nullptr*/) {
  unsigned long result = 0;
  do {
...
std::string Win::PathnamePathAppendA(const std::string &pathname,
...
std::wstring Win::PathnamePathAppendW(const std::wstring &pathname,
...
bool Win::GetFileNameAndFormat(_In_ const std::string &pathname,
                               _Out_ std::string &out_name,
                               _Out_ std::string &out_format) {
  bool result = false;
  out_name.clear();
  out_format.clear();
  do {
...
bool Win::EnableDebugPrivilege(const TCHAR *privilegeName,
...
std::string Win::EnumCorrectPath(const std::string &in,
  ...
  do {
    ...
    if (found != std::string::npos) {
      result.replace(found, strlen(replace_at == '\\' ? "//" : "\\\\"),
                     sizeof(replace_at), replace_at);
      continue;
    }
    found = result.find(replace_at == '\\' ? "\\\\\\" : "///");
    if (found != std::string::npos) {
    ...
    if (found != std::string::npos) {
      result.replace(found, sizeof(replace_at) * 2, sizeof(replace_at),
                     replace_at);
      continue;
    }
    found = result.find({replace_at, replace_at, replace_at});
    if (found != std::string::npos) {
      result.replace(found, sizeof(replace_at) * 3, sizeof(replace_at),
                     replace_at);
      continue;
    }
    break;
...
void Win::FindFileAssignPathOnce(const std::string &path,
  ...
  do {
    ...
    shared::Win::EnumFolder(
        path, folders, files, "*.*", true,
        [&](const std::string &pathname, const std::string &identify,
            const bool &is_directory) {
          bool just_is = false;
          do {
            if (is_directory)
              break;
            const std::string current_target_fullpathname =
                shared::Win::PathFixedA(path + "\\" + pathname);
            for (const auto &target_fname : ffname_s) {
              if (::_memicmp(target_fname.c_str(), identify.c_str(),
                             target_fname.size()) == 0) {
...
void Win::FindFileAssignPath(const std::string &path,
                             const std::vector<std::string> &ffname_s,
                             tfEnumFolderNode &found_s) {
  found_s.clear();
  do {
...
void Win::EnumFolder(const std::string &Path, tfEnumFolderNode &Folders,
                     tfEnumFolderNode &Files,
                     const char *FileFilter /*= "*.*"*/,
                     bool bSleepDirect /*= false*/,
                     const tfEnumFolderCb &enumcb /*= nullptr*/) {
  EnumFoldersAndFiles(
      Path, Folders, Files, FileFilter, bSleepDirect,
...
void Win::EnumFoldersAndFiles(
    ...
    const char *FileFilter /*= "*.*"*/, bool bSleepDirect /*= false*/,
    const std::function<void(const std::string &, const std::string &,
                             const _finddata_t &)> &enumcb /*= nullptr*/) {
  if (Path.empty())
    return;
  ...
  do {
    ...
    Paths.erase(begin);
    if (*std::prev(TargetPath.end()) != '\\' &&
        *std::prev(TargetPath.end()) != '/') {
        ...
        if (bSleepDirect) {
          if (FindData.attrib & _A_SUBDIR) {
            ...
            if (havedone.find(the) == havedone.end()) {
              ...
              Folders.emplace(
                  std::make_pair(EnumCorrectPath(the), FindData.name));
              if (enumcb) {
            ...
            auto the = path_pos + "\\" + FindData.name;
            the.erase(the.find(Path), Path.size());
            Files.emplace(std::make_pair(EnumCorrectPath(the, '\\', false),
                                         FindData.name));
            if (enumcb) {
              enumcb(the, FindData.name, FindData);
            }
          ...
          the.erase(the.find(Path), Path.size());
          Files.emplace(
              std::make_pair(EnumCorrectPath(the, '\\', false), FindData.name));
          if (enumcb) {
            enumcb(the, FindData.name, FindData);
          }
        }
...
bool Win::WriteConsoleInput(const int &input_char) {
  ...
  do {
    ...
    ir[0].Event.KeyEvent.wVirtualScanCode =
        ::MapVirtualKey(input_char, MAPVK_VK_TO_VSC);
    ...
    ir[1].Event.KeyEvent.wVirtualScanCode =
    ...
    ir[2].Event.KeyEvent.wVirtualScanCode =
        ::MapVirtualKey(VK_RETURN, MAPVK_VK_TO_VSC);
...
std::wstring Win::GeneratePathKeyW(const std::wstring &path) {
  std::string result = GeneratePathKeyA(IConv::WStringToMBytes(path));
  return IConv::MBytesToWString(result);
}
std::string Win::GeneratePathKeyA(const std::string &path) {
  std::string result = path;
  do {
    if (result.empty())
      break;
    ...
    result.erase(std::remove(result.begin(), result.end(), ':'), result.end());
    result.erase(std::remove(result.begin(), result.end(), '.'), result.end());
    result = IConv::ToLowerA(result);
    std::reverse(result.begin(), result.end());
  } while (0);
  return result;
}
bool Win::ComparePathOrPathname(const std::string &path1,
...
std::wstring Win::GuidW() {
  ...
  do {
    ...
    if (S_OK != ::CoCreateGuid(pGuid))
      break;
    int nLen = ::swprintf_s(
        buffer, 512, L"%08X-%04X-%04X-%02X%02X-%02X%02X%02X%02X%02X%02X",
...
std::string Win::GuidA() {
  ...
  do {
    ...
    if (S_OK != ::CoCreateGuid(pGuid))
      break;
    int nLen = ::sprintf_s(
        buffer, 512, "%08X-%04X-%04X-%02X%02X-%02X%02X%02X%02X%02X%02X",
        pGuid->Data1, pGuid->Data2, pGuid->Data3, pGuid->Data4[0],
        pGuid->Data4[1], pGuid->Data4[2], pGuid->Data4[3], pGuid->Data4[4],
        pGuid->Data4[5], pGuid->Data4[6], pGuid->Data4[7]);
    if (nLen <= 0)
...
bool Win::RemoveStrlW(const std::wstring &src, const std::wstring &dest,
                      std::wstring &out) {
  bool result = false;
  out.clear();
  do {
...
bool Win::RemoveStrlA(const std::string &src, const std::string &dest,
                      std::string &out) {
  bool result = false;
  out.clear();
  do {
  ...
  for (int i = 0; i < __argc; ++i) {
    ...
    do {
      ...
      if (key.empty() || value.empty())
        continue;
      out_param_map.emplace(shared::IConv::ToLowerA(std::string(k, v - k)),
                            std::string(v + ::strlen("=")));
    } while (0);
  }
}
...
#if 0 //!@ by ---
      //! https://stackoverflow.com/questions/47315191/how-to-check-group-membership-for-a-process-that-is-not-self
...
bool Win::IsProcessHighIntegrity(const DWORD &pid) {
  ...
  do {
    ...
    if (!hProcess)
      break;
    ::OpenProcessToken(hProcess, TOKEN_ALL_ACCESS, &hToken);
    if (!::DuplicateTokenEx(hToken, TOKEN_QUERY, NULL, SecurityImpersonation,
                            TokenImpersonation, &hTokenCopy))
      break;
    if (!hTokenCopy)
      break;
    SID_IDENTIFIER_AUTHORITY NtAuthority = SECURITY_NT_AUTHORITY;
...
bool Win::ParseCommandLineParameters(const std::string &commandline,
                                     tfCommandLineNode &out) {
  bool result = false;
  out.clear();
  do {
...
bool Win::ParseCommandLineParameters(const int &argc, char **argv,
                                     tfCommandLineNode &out) {
  bool result = false;
  out.clear();
  do {
...
bool FileUpdateObj::operator<<(const std::string &jsonObjNode) {
  ...
  do {
    ...
    if (doc.ObjectEmpty())
      break;
    if (!doc.HasMember("Identify") || !doc["Identify"].IsString() ||
        doc["Identify"].GetStringLength() <= 0)
      break;
    ...
    if (doc.HasMember("OriginalFileFormat") &&
        doc["OriginalFileFormat"].IsString())
...
void FileUpdateObj::operator>>(std::string &jsonObjectString) const {
  ...
  doc.AddMember(
      ...
      rapidjson::Value()
          .SetString(Identify.empty() ? "" : Identify.c_str(),
                     doc.GetAllocator())
          .Move(),
      doc.GetAllocator());
  doc.AddMember(
      rapidjson::Value().SetString("DiskFileSize", doc.GetAllocator()).Move(),
      rapidjson::Value().SetUint(DiskFileSize).Move(), doc.GetAllocator());
  doc.AddMember(
      rapidjson::Value().SetString("MD5", doc.GetAllocator()).Move(),
      rapidjson::Value()
          .SetString(MD5.empty() ? "" : MD5.c_str(), doc.GetAllocator())
          .Move(),
      doc.GetAllocator());
  doc.AddMember(
      rapidjson::Value().SetString("Version", doc.GetAllocator()).Move(),
      rapidjson::Value()
          .SetString(Version.empty() ? "" : Version.c_str(), doc.GetAllocator())
          .Move(),
      doc.GetAllocator());
  doc.AddMember(rapidjson::Value()
                    .SetString("OriginalFileFormat", doc.GetAllocator())
                    .Move(),
                rapidjson::Value()
                    .SetString(OriginalFileFormat.empty()
                                   ? ""
                                   : OriginalFileFormat.c_str(),
                               doc.GetAllocator())
                    .Move(),
                doc.GetAllocator());
  doc.AddMember(
      rapidjson::Value()
          .SetString("OriginalFileName", doc.GetAllocator())
          .Move(),
      rapidjson::Value()
          .SetString(OriginalFileName.empty() ? "" : OriginalFileName.c_str(),
                     doc.GetAllocator())
          .Move(),
      doc.GetAllocator());
...
bool Win::GetAssignUserSid(const std::string &user_name, std::string &outSid) {
  ...
  do {
    ...
    if (FALSE == ::LookupAccountNameA(nullptr, user_name.c_str(), UserSid,
                                      &dwUserSid, UserDomain, &dwUserDomain,
                                      &SidNameUser))
      break;
    ...
    if (!pSID_IDENTIFIER_AUTHORITY)
      break;
    outSid = std::format("S-{}-{}", SID_REVISION,
...
bool Win::GetCurrentUserSid(std::string &outSid) {
  ...
  do {
    if (FALSE == ::GetUserNameA(UserName, &dwUserName))
      break;
    SID_NAME_USE SidNameUser;
    if (FALSE == ::LookupAccountNameA(nullptr, UserName, UserSid, &dwUserSid,
                                      UserDomain, &dwUserDomain, &SidNameUser))
      break;
    PSID_IDENTIFIER_AUTHORITY pSID_IDENTIFIER_AUTHORITY =
        ::GetSidIdentifierAuthority(UserSid);
    if (!pSID_IDENTIFIER_AUTHORITY)
...
std::string Win::RealtimeSpeed(const long long &speed_bype_size,
  ...
  do {
    ...
    if (speed_bype_size >= 1024 * 1024 * 1024) { //!@ GB
      nSprintf =
          ::sprintf_s(&result[0], _MAX_PATH, "%.2f",
                      (speed_bype_size * 1.0) / (1024.0 * 1024.0 * 1024.0));
      result.resize(nSprintf);
      result.append(divide ? " GB" : "GB");
    } else if (speed_bype_size >= 1024 * 1024) { //!@ MB
      nSprintf = ::sprintf_s(&result[0], _MAX_PATH, "%.2f",
                             (speed_bype_size * 1.0) / (1024.0 * 1024.0));
      result.resize(nSprintf);
      result.append(divide ? " MB" : "MB");
    } else if (speed_bype_size >= 1024) { //!@ KB
      nSprintf = ::sprintf_s(&result[0], _MAX_PATH, "%.2f",
                             (speed_bype_size * 1.0) / 1024.0);
      result.resize(nSprintf);
      result.append(divide ? " KB" : "KB");
    } else { //!@ B
      nSprintf = ::sprintf_s(&result[0], _MAX_PATH, "%.2f",
                             __max(0.0, (speed_bype_size * 1.0) / 1.0));
      result.resize(nSprintf);
...
bool Win::OpenTargetPathnameLockedFile(const std::string &targetPathname) {
  ...
  do {
    ...
    result = nullptr !=
             ::ShellExecuteA(
                 NULL, "open", "explorer.exe", parameters.c_str(),
...
std::string Win::UrlFixedByDowntool(const std::string &utf8_url) {
  static const std::string urlstd =
  ...
  do {
    ...
    std::for_each(utf8_url.begin(), utf8_url.end(), [&](const auto &ch) {
    ...
    for (size_t i = 0; i < src2.size(); i += 2) {
      res.emplace_back(
          std::string("%").append(1, src2[i]).append(1, src2[i + 1]));
    }
    if (res.empty())
      break;
    ...
    std::for_each(src1.begin(), src1.end(),
                  [&](const auto &node) { result.append(node); });
  } while (0);
  return result;
}
...
using tfModifyFun = long(__stdcall *)(IPinnedList3 *, LPCITEMIDLIST,
                                      LPCITEMIDLIST, PLMC);
...
bool Win::taskbarpin(const std::string exe_or_lnk_pathname,
                     const bool &pin /*= true*/,
                     const std::string &lnk_pathname /*= ""*/) {
  ...
  case WindowsOS::Win10: {
    {
      ...
      do {
        ...
        if (!pItemidList)
          break;
        comret = ::CoCreateInstance(
            taskbarpinwin10::clsid_taskbandpin, NULL, CLSCTX_ALL,
            taskbarpinwin10::iid_ipinnedlist3, (LPVOID *)(&pinnedList));
        if (comret != S_OK || !pinnedList)
          break;
        comret = pinnedList->vtbl->Modify(pinnedList, pin ? NULL : pItemidList,
                                          pin ? pItemidList : NULL,
                                          taskbarpinwin10::PLMC::PLMC_EXPLORER);
        if (comret != S_OK)
          break;
        result = true;
      } while (0);
    ...
    setup_name = shared::IConv::MBytesToWString(
        shared::Win::GetNameByPathnameA(exe_or_lnk_pathname));
    setup_path = shared::IConv::MBytesToWString(shared::Win::PathFixedA(
        shared::Win::GetPathByPathnameA(exe_or_lnk_pathname)));
    if (setup_path.empty() || setup_name.empty())
      return false;
    bool exists = false;
    const std::string TaskBarPath =
        shared::Win::GetSpecialFolderLocationA(CSIDL_APPDATA) +
        R"(\Microsoft\Internet Explorer\Quick Launch\User Pinned\TaskBar\)";
    std::map<std::string, std::string> enum_files{}, enum_dirs{};
    shared::Win::EnumFoldersAndFiles(
        TaskBarPath, enum_dirs, enum_files, "*.lnk", false,
        [&](const auto &path, const auto &name, const auto &data) {
          ...
          if (rfdot1 != std::string::npos)
            find_name[rfdot1] = 0;
          auto lnkpath = shared::Win::GetLnkFormPathW(
              shared::IConv::MBytesToWString(TaskBarPath + name));
#ifdef _UNICODE
          std::string find_exepath = shared::Win::GetNameByPathnameA(
              shared::IConv::WStringToMBytes(lnkpath));
#else
          std::string find_exepath =
              shared::Windows::GetNameByPathname(lnkpath);
#endif
      ...
      do {
        ...
        LSTATUS regResult =
            regKey.Open(HKEY_LOCAL_MACHINE,
                        _T("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Expl")
                        _T("orer\\CommandStore\\shell\\Windows.taskbarpin"),
                        KEY_READ);
        if (ERROR_SUCCESS != regResult)
          break;
        regResult = regKey.QueryStringValue(_T("ExplorerCommandHandler"), szVal,
                                            &uValSize);
        if (ERROR_SUCCESS != regResult) {
          regKey.Close();
          break;
        }
        regResult = regKey.Create(HKEY_CURRENT_USER,
                                  _T("SOFTWARE\\Classes\\*\\shell\\{:}"));
        if (ERROR_SUCCESS != regResult) {
          regKey.Close();
          break;
        }
        ...
        comret = ::CoCreateInstance(CLSID_Shell, NULL, CLSCTX_SERVER,
                                    IID_IShellDispatch, (LPVOID *)&pShellDisp);
        if (!SUCCEEDED(comret))
          break;
        comret =
            pShellDisp->NameSpace(CComVariant(setup_path.c_str()), &pFolder);
        if (!SUCCEEDED(comret))
          break;
        ...
        if (!SUCCEEDED(comret))
          break;
        regResult =
            regKey.Open(HKEY_CURRENT_USER, _T("SOFTWARE\\Classes\\*\\shell"),
                        KEY_READ | KEY_WRITE);
        if (ERROR_SUCCESS != regResult)
          break;
  ...
  default: {
    ...
    do {
      if (pin)
        ShellExecuteA(NULL, "taskbarpin",
                      shared::Win::PathFixedA(lnk_pathname).c_str(), NULL, NULL,
                      SW_HIDE);
      else
        ShellExecuteA(NULL, "taskbarunpin",
                      shared::Win::PathFixedA(lnk_pathname).c_str(), NULL, NULL,
                      SW_HIDE);
    } while (0);
    result = true;
...
bool Win::CreateShortcutUrl(
  ...
  if (BindLnkUrl.empty() || BindLnkIcoPathname.empty() ||
      BindLnkPathname.empty())
    return false;
  HRESULT comret = S_FALSE;
  bool result = false;
  ...
  do {
    comret = ::CoCreateInstance(CLSID_ShellLink, NULL, CLSCTX_INPROC_SERVER,
                                IID_IShellLink, (void **)&pIShellLinkW);
    if (S_OK != comret)
      break;
    comret =
        pIShellLinkW->QueryInterface(IID_IPersistFile, (void **)&pIPersistFile);
    if (S_OK != comret)
      break;
    comret = pIShellLinkW->SetPath(IConv::MBytesToWString(BindLnkUrl).c_str());
    if (S_OK != comret)
      break;
    comret = pIShellLinkW->SetIconLocation(
        IConv::MBytesToWString(PathFixedA(BindLnkIcoPathname)).c_str(), 0);
    if (S_OK != comret)
      break;
    comret = pIPersistFile->Save(
        IConv::MBytesToWString(PathFixedA(BindLnkPathname)).c_str(), TRUE);
    if (S_OK != comret)
      break;
    result = true;
  } while (0);
...
bool Win::CreateShortcut(const std::string &szPath, const std::string &szLink,
                         const std::string &szIcoPath /*= ""*/,
                         const std::string &szArgument /*= ""*/) {
  HRESULT comret = S_FALSE;
  IShellLinkW *pIShellLinkW = nullptr;
  IPersistFile *pIPersistFile = nullptr;
  bool result = false;
  do {
    ...
    if (comret != S_OK)
      break;
    comret =
        pIShellLinkW->SetPath(shared::IConv::MBytesToWString(szPath).c_str());
    if (comret != S_OK)
      break;
    comret = pIShellLinkW->SetWorkingDirectory(
        shared::IConv::MBytesToWString(PathFixedA(GetPathByPathnameA(szPath)))
            .c_str());
    if (comret != S_OK)
      break;
    if (!szIcoPath.empty())
      comret = pIShellLinkW->SetIconLocation(
          shared::IConv::MBytesToWString(szIcoPath).c_str(), 0);
    if (comret != S_OK)
      break;
    if (!szArgument.empty())
      comret = pIShellLinkW->SetArguments(
          shared::IConv::MBytesToWString(szArgument).c_str());
    if (comret != S_OK)
      break;
    comret =
        pIShellLinkW->QueryInterface(IID_IPersistFile, (void **)&pIPersistFile);
    if (comret != S_OK)
      break;
    comret = pIPersistFile->Save(shared::IConv::MBytesToWString(szLink).c_str(),
                                 TRUE);
    if (comret != S_OK)
      break;
...
bool Win::GdiplusGetEncoderClsid(const std::wstring &MimeTypeFormat,
                                 CLSID &outClsid) {
  bool result = false;
...
bool Win::GdiplusCaptureScreen(
    std::string &out_image_buffer,
    const std::wstring &MimeTypeFormat /*= LR"(image/png)"*/) {
  bool result = false;
#if ENABLE_WIN_DEFINE_GDIPLUS
  ...
  do {
    ...
    hCaptureBitmap =
        ::CreateCompatibleBitmap(hDesktopDC, sizeWindow.cx, sizeWindow.cy);
    ::SelectObject(hCaptureDC, hCaptureBitmap);
    if (FALSE == ::BitBlt(hCaptureDC, 0, 0, sizeWindow.cx, sizeWindow.cy,
    ...
    if (!SUCCEEDED(
            ::IStream_Read(pIStream, &out_image_buffer[0], liSize.LowPart)))
      break;
  } while (0);
...
void Win::FullScreenSize(SIZE &size) {
  ...
  do {
    DEVMODE DEVMODE__;
    if (FALSE ==
        ::EnumDisplaySettingsW(NULL, ENUM_CURRENT_SETTINGS, &DEVMODE__))
      break;
...
bool Win::FullScreenSnapshootToBMP(std::string &bmpBuffer) {
  ...
  do {
    ...
    if (FALSE ==
        ::StretchBlt(hmdcScreen, 0, 0, x, y, hdcScreen, 0, 0, x, y, SRCCOPY))
      break;
    ...
    if (!hMemBmp)
      break;
    if (S_OK != ::CreateStreamOnHGlobal(hMemBmp, FALSE,
                                        &pStmImage)) // 将内存区B作为流的起始
      break;
    ...
    unsigned char *bitmapBits =
        new unsigned char[fbi.info.bmiHeader.biSizeImage];
    if (!bitmapBits)
      break;
    ::memset(bitmapBits, 0x00, fbi.info.bmiHeader.biSizeImage);
    ::GetDIBits(hmdcScreen, hBitmap, 0, y, bitmapBits, &fbi.info,
                DIB_RGB_COLORS);
...
HICON Win::CreateIcon(const std::string &icoCache) {
  ...
  do {
    ...
    if (offset != 0)
      result = ::CreateIconFromResourceEx(pData + offset, dwSize, TRUE, 0x30000,
                                          0, 0, LR_SHARED);
  } while (0);
  return result;
}
bool Win::MainWindowLoadIcon(const HWND &hwnd_main,
                             const std::string &icon_buffer) {
  bool result = false;
  HICON hIcon = nullptr;
  do {
...
#if ENABLE_WIN_DEFINE_RAPIDJSON
bool Win::CompareFileUpdateObjs(const FileUpdateObjs &objs1,
                                const FileUpdateObjs &objs2) {
  bool equal = false;
  do {
...
DWORD Win::GetWindowsNTReleaseId() {
  ...
  do {
    std::string read;
    if (!Win::Registry::NtRead(
            R"(HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ReleaseId)",
            read))
      break;
    if (read.empty())
      break;
...
#if ENABLE_WIN_DEFINE_RAPIDJSON
bool Win::GenerateFileUpdateObjs(
    const std::string &gbk_json_update_config_arrsy, FileUpdateObjs &outObjs) {
  bool result = false;
  outObjs.clear();
  do {
    if (gbk_json_update_config_arrsy.empty())
      break;
    rapidjson::Document doc;
    if (doc.Parse(gbk_json_update_config_arrsy.data(),
                  gbk_json_update_config_arrsy.size())
            .HasParseError())
      break;
    if (!doc.IsArray())
      break;
  ...
  do {
    ...
    for (const auto &node : MAP_IMAGE_SIGNATURES) {
      ...
      std::string file_buffer =
          File::ReadAssignSize(filePathname, node.second.size());
      if (file_buffer.empty())
        continue;
      if (0 !=
          ::memcmp(&node.second[0], file_buffer.data(), node.second.size()))
        continue;
      result = node.first;
      break;
    }
    ...
    for (const auto &node : MAP_FILE_SIGNATURES) {
      ...
      case EN_FILE_TYPE::_RAR: {
        ...
        if (find_rar4 != MAP_FILE_SIGNATURES.end()) {
          file_buffer =
              File::ReadAssignSize(filePathname, find_rar4->second.size());
          if (file_buffer.empty())
            break;
          if (0 == ::memcmp(&node.second[0], file_buffer.data(),
                            node.second.size())) {
        ...
        if (find_rar5 != MAP_FILE_SIGNATURES.end()) {
          file_buffer =
              File::ReadAssignSize(filePathname, find_rar5->second.size());
          if (file_buffer.empty())
            break;
          if (0 == ::memcmp(&node.second[0], file_buffer.data(),
                            node.second.size())) {
...
void Win::MainProcess(
    const std::function<void(const std::string &input, bool &exit)>
        &main_process_callback,
  ...
  std::thread([&]() {
    do {
      ...
      std::cin >> std::skipws;
      main_process_callback(lowercase ? IConv::ToLowerA(_input) : _input,
                            exit_flag);
      if (exit_flag)
        break;
    } while (1);
...
extern const std::map<EN_FILE_TYPE, std::vector<std::uint8_t>>
    MAP_FILE_SIGNATURES = {
  ...
  unsigned char rawData[6] =
  {
        ...
        */
        ...
        /*
        4D 5A	 	MZ
        COM, DLL, DRV, EXE, PIF, QTS, QTX, SYS	 	Windows/DOS executable
        file (See The MZ EXE File Format page for the structure of an EXE file,
        with coverage of NE, TLINK, PE, self-extracting archives, and more.)
        Note: MZ are the initals of Mark Zbikowski, designer of the DOS
        executable file format. ACM	 	MS audio compression manager
        driver AX	 	Library cache file CPL	 	Control panel
        application FON	 	Font file OCX	 	ActiveX or OLE Custom
        Control OLB	 	OLE object library SCR	 	Screen saver VBX
        VisualBASIC application VXD, 386	 	Windows virtual device
        drivers
        */
        {EN_FILE_TYPE::_WINPE,
         std::vector<std::uint8_t>{
        ...
        // ISO-9660 CD Disc Image (This signature usually   occurs at byte 8001,
        // 8801, or 9001.)
        {EN_FILE_TYPE::_ISO,
         std::vector<std::uint8_t>{
             0x43,
        ...
        // BZIP Archive bzip2 compressed archive [BZ2; TAR.BZ2; TBZ2; TB2; (bz;
        // bz2)]
        {EN_FILE_TYPE::_BZIP2,
         std::vector<std::uint8_t>{
             0x42,
...
extern const std::map<EN_IMAGE_TYPE, std::vector<std::uint8_t>>
...
const std::map<EnSystemProtectedProcessType, std::wstring>
    mapSystemProtectedProcessImageName = {
