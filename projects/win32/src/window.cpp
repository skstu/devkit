...
namespace shared {
 ...
 bool Win::Window::EnumTargetWndOfChildWnd(const HWND& target_wnd, std::vector<HWND>& child_wnd_s) {
  bool result = false;
  child_wnd_s.clear();
  do {
   if (!target_wnd)
   ...
   ::EnumChildWindows(target_wnd,
    ...
    }, reinterpret_cast<LPARAM>(&child_wnd_s));
   result = true;
  } while (0);
  return result;
 }
 HICON Win::Window::LoadMemIcon(const std::string& input_buffer) {
  HICON result = nullptr;
  do {
   if (input_buffer.empty())
    break;
   auto pIconBuffer = (PBYTE)input_buffer.data();
   if (!pIconBuffer)
    break;
   int offset = ::LookupIconIdFromDirectoryEx(pIconBuffer, TRUE, 0, 0, LR_DEFAULTCOLOR);
   if (offset == 0)
    break;
   result = ::CreateIconFromResourceEx(
    pIconBuffer + offset,   // Pointer to the icon data
    input_buffer.size(), // Size of the icon data
    TRUE,                  // Indicates that the resource is an icon
    0x00030000,            // Icon version and format flags
    0,                     // Width (use default)
    0,                     // Height (use default)
    LR_DEFAULTCOLOR        // Load the icon using the default color format
   );
  } while (0);
  return result;
 }
